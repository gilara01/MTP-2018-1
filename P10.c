...
void cadastra(Produto * p){
    ...
    getchar();
    printf("\nDigite a largura: ");
    scanf("%f", &(p->dimensoes.larg));
    getchar();
    printf("\nDigite a profundidade: ");
    scanf("%f", &(p->dimensoes.profu));
    getchar();
    printf("\nDigite a altura: ");
    scanf("%f", &(p->dimensoes.alt));
    getchar();
}
...
void consulta(Produto * p){
    ...
    printf("\n%s, R$ %.2f, L: %.2fm x P: %.2fm x A: %.2fm\n", p->nome, p->preco,(p->dimensoes.larg),(p->dimensoes.profu), (p->dimensoes.alt));
    while((c=getchar())!='\n');
}
...
int main(){
    ...
    while(opt1 != 0){
        ...
        printf("\n1. Consultar\n2. Cadastrar novo\n3. Carregar arquivo para memoria (sobrescreve memoria)\n4. Salvar memoria em arquivo (sobrescreve arquivo)\n0. Sair\n ");
        scanf("%d", &opt1);
        getchar();
        opt2 = -1;
        switch(opt1){
