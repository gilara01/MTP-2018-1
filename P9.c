...
void cadastro(struct produto vetp[], int i)
{
	...
	getchar();
	printf("\n Digite a largura: ");
	scanf("%f", &vetp[i].end.larg);
	getchar();
	printf("\n Digite a profundidade: ");
	scanf("%f", &vetp[i].end.profu);
	getchar();
	printf("\n Digite a altura: ");
	scanf("%f", &vetp[i].end.alt);
	getchar();
}
void pesquisa(struct produto vetp[], int i)
{
	...
	{
		printf("\n %s , R$ %.2f, L: %.2fm x P: %.2fm x A: %.2fm", vetp[i].nome, vetp[i].preco, vetp[i].end.larg, vetp[i].end.profu, vetp[i].end.alt);
	}
}
int main()
{
